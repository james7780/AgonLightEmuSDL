...
int EMSCRIPTEN_KEEPALIVE emu_send_variables(const char *const *files, int num, int location,
    ...
    if (!argv) {
        printf("[eZ80-Emu] Transfer Error: can't allocate transfer commands... wut\n");
        return LINK_ERR;
    }
    ...
    if (err != 0) {
        printf("[eZ80-Emu] USB transfer error code %d.\n", err);
    }
...
w_err:
    (void)fclose(fd);
    if(remove(file)) {
        printf("[eZ80-Emu] Transfer Error: Please contact the developers\n");
    }
    return LINK_ERR;
}
