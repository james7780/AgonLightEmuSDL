...
static void watchdog_event(enum sched_item_id id) {
    ...
    if (watchdog.control & 1) {
        ...
        if (watchdog.control & 4) {
            printf("[eZ80-Emu] Watchdog NMI triggered.\n");
            cpu_nmi();
        }
...
void watchdog_reset() {
    ...
    printf("[eZ80-Emu] Watchdog timer reset.\n");
}

eZ80portrange_t init_watchdog(void) {
    printf("[eZ80-Emu] Initialized Watchdog Timer...\n");
    return pwatchdog;
}
...
eZ80portrange_t init_protected(void) {
    printf("[eZ80-Emu] Initialized Protected Ports...\n");
    return p9xxx;
}
