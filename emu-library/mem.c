...
void mem_init(void) {
    ...
    mem.ram.block = memory;

    mem.flash.write = 0;
    mem.flash.command = FLASH_NO_COMMAND;
    printf("[eZ80-Emu] Initialized Memory...\n");
}
...
static void flash_erase(uint32_t addr, uint8_t byte) {
    ...
    printf("[eZ80-Emu] Erased Unlocked Sectors.\n");
}
...
//         printf("[eZ80-Emu] NMI reset caused by writing to the stack limit at address %#06x. Hint: Probably a stack overflow (aka too much recursion).\n", addr);
//         cpu_nmi();
...
//         printf("[eZ80-Emu] NMI reset caused by writing to protected memory (%#06x through %#06x) at address %#06x from unprivileged code.\n", control.protectedStart, control.protectedEnd, addr);
//         cpu_nmi();
...
//                 if (unprivileged_code()) {
//                     control.protectionStatus |= 2;
//                     printf("[eZ80-Emu] NMI reset caused by writing to flash at address %#06x from unprivileged code. Hint: Possibly a null pointer dereference.\n", addr);
//                     cpu_nmi();
